...
/**
 * Class generate address data.
 *
 * @package PHPFaker
 * @subpackage Generator
 */
class AddressGenerator extends AbstractGenerator
{
    ...
    {
        $method = $this->random(self::$_street_name_formats);
        $result[] = $this->getGenerator('name')->$method();
    ...
    {
        return $this->numerify(implode( " ", array('#####', $this->streetName())));
    }

    public function abodeAddress($includeStreet = false)
    {
        if ( $includeStreet ) {
            $str[] = '#####';
        }
        ...
        $str[] = $this->random($formats);
        if ($includeStreet) {
            $str[] = $this->streetName();
        }
        return $this->numerify(implode(" ", $str ));
    }
    ...
    {
        return $this->random(self::$_uk_counties);
    }

    public function ukCountry()
    {
        return $this->random(self::$_uk_countries);
    }

    public function postCode()
    {
        $a = $this->random(self::$_uk_postcode_formats);
        $result = $this->bothify($a);
    ...
    {
        return $this->random(self::$_us_states);
    }

    public function usStateAbbr()
    {
        return $this->random(self::$_us_states_abbr);
    }

    public function zipCode()
    {
        $a = $this->random(self::$_us_zipcode_formats);
        $result = $this->numerify($a);
        return $result;
    }
}
